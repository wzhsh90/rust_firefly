...
impl ComService {
    pub async fn exist_name(&self, name: &String) -> Result<u64> {
        let exist_name_sql = load_sql("exist_name");
        BASE_DB.py_fetch("", exist_name_sql, &json!({"name": name })).await
    }
    ...
    pub async fn update(&self, arg: &Company) -> Result<DBExecResult> {
        let update_sql = load_sql("update");
        BASE_DB.py_exec("", update_sql, arg).await
    }

    pub async fn list(&self, name: &str, page_index: u64, page_size: u64) -> Result<Page<Company>> {
        let list_sql = load_sql("list");
